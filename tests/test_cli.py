...
def test_cli_levels_invalid_value_returns_2(tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert rc == 2
    assert "invalid level 'BOGUS'" in captured.err
...
def test_cli_date_summary_limit(capsys):
    from log_analyzer.cli import main
...
def test_cli_date_summary_limit_reverse(capsys):
    from log_analyzer.cli import main
...
def test_contains_filter(tmp_path, capsys):
    from log_analyzer.cli import main
...
def test_contains_multiple_keywords(tmp_path, capsys):
    from log_analyzer.cli import main
